...
def test_voice_spec_defaults_and_threading_from_host(tmp_path):
    ...
    host = Host(id="tessa", name="Tessa", role="explainer", cfg_weight=0.35, speech_rate=0.85)
    spec = voice_spec_for(host, tmp_path)
    assert spec.cfg_weight == 0.35 and spec.speech_rate == 0.85

    guest = Guest(id="hanna", name="Hanna", cfg_weight=0.4, speech_rate=0.9)
    spec2 = voice_spec_for(guest, tmp_path)
    assert spec2.cfg_weight == 0.4 and spec2.speech_rate == 0.9
...
def test_audition_sweeps_every_combination(tmp_path):
    ...
    rows = audition("Dit is een test paragraaf voor de audition.", {"tessa": ref}, tmp_path / "out", NullSynth(),
                    exaggerations=(0.4, 0.6), cfg_weights=(0.3, 0.5), speech_rates=(1.0,))
    assert len(rows) == 4  # 1 speaker x 2 exaggerations x 2 cfg_weights x 1 speech_rate
    combos = {(r["exaggeration"], r["cfg_weight"], r["speech_rate"]) for r in rows}
    assert combos == {(0.4, 0.3, 1.0), (0.4, 0.5, 1.0), (0.6, 0.3, 1.0), (0.6, 0.5, 1.0)}
    assert len({r["path"] for r in rows}) == 4  # distinct filenames, nothing overwritten
