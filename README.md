...
- **Thorough plan and lexicon.** With the local backend, the content plan and lexicon are made section by section instead of in one call over the whole chapter (`STUDIEPODCAST_PLAN_MODE=thorough`, the default for `local`). Each section yields candidate claims, definitions and misconceptions. Every quote has to be near-literal in that section's text; one that isn't goes back to be fixed, and a claim whose quote still isn't there is dropped with a warning. A chapter pass then picks and merges the claims by candidate id, so quotes are never rewritten, and writes the summary and learning objectives. `STUDIEPODCAST_PLAN_REVIEW_ROUNDS` (default 1) review rounds add what's missing and remove what's double. It takes roughly one call per section plus three or four more. `studiepodcast plan <book> <chapter> --compare` makes the plan both ways and writes `plans/<chapter>.plan-compare.md`: sections covered, how many quotes are literally in the source, claims per section, and both plans in full.
...
| `STUDIEPODCAST_PLAN_MODE`, `STUDIEPODCAST_PLAN_REVIEW_ROUNDS` | `single` (`thorough` with the local backend), `1` | How the content plan and lexicon are made |
